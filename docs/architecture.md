...
The catalogue is split in two. Most keys only change what the adapter _reads_. Eight of them — the composer, the Post, Like and Unlike buttons, the tab bar, "Show more", dialogs and toasts — are what a tool clicks, types into, or reads back to confirm an action, so moving one changes what the app does on the account. Those are `locked`: no tool can set them, only the user editing the file, and each such override is logged when the file loads. Repairing a read selector is a loop of tools rather than a guess: `xpilot_list_selectors` to see them, `x_inspect_page` for the markup X actually renders, `xpilot_test_selector` to measure a candidate, `xpilot_set_selector` to write it once the browser has accepted it.
