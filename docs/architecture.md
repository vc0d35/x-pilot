...
Those selectors are configurable. `src/shared/selectors.ts` holds the shipped defaults and a line per key saying what it points at; the preload's `SEL` is a copy of them that `applySelectorOverrides` rewrites in place from `<userData>/selectors.json` before any tool runs and again whenever the file changes, so the call sites never know an override happened. Only overridden keys are stored, each alongside the default it replaced. That is the sync rule: a new version's updated defaults flow through for every key nobody touched, while an override whose recorded default has since changed is kept and reported `stale`, so nothing the user fixed is silently discarded and nothing new is silently ignored. Overrides go to every X view, visible and hidden, because the adapter must read the page the same way wherever it runs.
...
| app tools      | main, app services           | `xpilot_search_history`, `xpilot_save_article_pdf`, `xpilot_list_library`, `xpilot_schedule_task`, `xpilot_read_page_styles`, `xpilot_write_page_styles`, `xpilot_list_selectors`, `xpilot_set_selector`, `xpilot_reset_selector` |
