...
- [ ] **Release provenance.** Build attestation and Dependabot are in: `release.yml` runs `actions/attest-build-provenance` (pinned, v2.4.0) over `dist/*.dmg` and `dist/*.zip` with `id-token`/`attestations` write on the job, so a download is checked with `gh attestation verify <file> --repo vc0d35/x-pilot`; `.github/dependabot.yml` watches `github-actions` and `npm` weekly with minor and patch grouped. Still to do: split build and sign into separate jobs.
- [ ] **Web search as an egress channel.** Queries are visible in the sidebar, which is the current mitigation; consider a setting to confirm searches during autonomous runs. Unattended runs are already narrowed: a scheduled task carries its own `webSearch` flag, off unless it was created asking for it, and `TaskRunner` passes `disabled` otherwise. Interactive turns are unchanged.
...
- [x] **Transcript retention.** `settings.history` holds `keepConversations` (default 200) and `keepDays` (default 90); `HistoryStore.applyRetention` deletes conversations past either limit with their events, never touching the live thread, one updated in the last hour, or a thread a scheduled task resumes. Settings shows the database size and the two limits, from `HistoryStore.stats()`.
