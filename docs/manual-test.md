...
- [ ] With Settings → Agent likes on Autonomous, click a kept view's own "like" button → no card; the like happens and the transcript shows a `view:x_like_post` row. Switch likes to Confirm → the card comes back, headed "Requested by the custom view "<name>"".
...
- [ ] Images: on a post with a screenshot or a chart, "what does this image say?" → a `x_view_image` row in the sidebar and an answer that quotes what is in the picture. Repeat with a custom view on screen and a post with an image in focus. On a Codex model without image input the tool fails saying so.
