...
describe('defineTool', () => {
  ...
  const tool = defineTool({
    name: 'xpilot_demo',
    description: 'demo',
    args: z.strictObject({ url: z.string(), limit: z.int().min(1).max(10).default(3), mode: z.enum(['a', 'b']).optional().describe('which one') }),
    annotations: { readOnlyHint: true },
    execute: async (args, _ctx: void) => { seen.push(args); return ok(args); },
  });
  ...
  it('derives the input schema from the arguments, with no $schema and nothing required by a default', () => {
    expect(tool.spec).toEqual({
      ...
      inputSchema: {
        type: 'object',
        properties: { url: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 10, default: 3 }, mode: { type: 'string', enum: ['a', 'b'], description: 'which one' } },
        required: ['url'],
  ...
  it('runTool fails with the issues instead of throwing, and does not run the tool', async () => {
    ...
    expect(await runTool(tool, { limit: 99 }, undefined)).toEqual(fail('Invalid arguments for xpilot_demo: url: Invalid input: expected string, received undefined; limit: Too big: expected number to be <=10'));
