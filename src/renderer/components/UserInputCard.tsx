 ...
 */
export function UserInputCard({ request, resolved, onResolve, cardRef }: {
  ...
  return (
    <div className="approval approval-input" ref={cardRef}>
      ...
      {request.questions.map((q) => (resolved
        ? <div key={q.id} className="input-question"><span className="input-prompt">{q.prompt}</span></div>
