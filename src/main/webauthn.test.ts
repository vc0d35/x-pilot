...
describe('configureTouchIdPasskeys', () => {
  function setup(group: string | null) {
    ...
    const log = vi.fn();
    const enabled = configureTouchIdPasskeys({ app, onSelectAccount: (l) => { session.on('select-webauthn-account', l); }, group, log });
  ...
  it('configures Touch ID and auto-selects a single account', () => {
    ...
    expect(app.configureWebAuthn).toHaveBeenCalledWith({ touchID: { keychainAccessGroup: 'T.com.vicnicius.xpilot.webauthn', promptReason: 'sign in to $1' } });
    const cb = vi.fn();
  ...
  it('picks the most recently used account when several match and logs the choice', () => {
    ...
    const cb = vi.fn();
    session.emit('select-webauthn-account', { preventDefault() {} }, { accounts: [{ name: 'old', credentialId: 'a' }, { name: 'new', credentialId: 'b' }] }, cb);
