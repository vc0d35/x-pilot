...
describe('xpilot_save_article_pdf', () => {
  ...
  it('rejects non-post urls and surfaces export errors', async () => {
    ...
    expect(await savePdf.execute({ url: 'https://x.com/alice/status/1' }, c)).toEqual(
      fail('PDF export failed: No post rendered before the timeout'),
    );
  });
});
...
describe('library tools', () => {
  it('lists items and opens only paths inside the library dir', async () => {
    const c = ctx();
    c.store.addLibraryItem({ postId: null, url: 'u', path: '/lib/a.pdf', title: 'A' });
    expect((await runTool(listLibrary, {}, c)) as { content: unknown[] }).toMatchObject({
      content: [expect.objectContaining({ path: '/lib/a.pdf' })],
    });
  ...
  it('still opens an item recorded under a previous library folder', async () => {
    const c = ctx();
    c.store.addLibraryItem({ postId: null, url: 'u', path: '/old-folder/b.pdf', title: 'B' });
    ...
    expect(await openPdf.execute({ path: '/old-folder/c.pdf' }, c)).toEqual(
      fail('Refusing to open a file that is not a PDF in the library'),
    );
  });
});
