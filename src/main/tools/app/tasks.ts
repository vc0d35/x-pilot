...
export const scheduleTask = defineTool({
  ...
  execute: async (args, ctx: AppToolCtx) => attempt(() => ctx.tasks.create({ title: args.title, prompt: args.prompt, schedule: args.schedule, threadMode: args.threadMode ?? 'resume', webSearch: args.webSearch === true })),
});
...
export const updateTask = defineTool({
  ...
  args: z.strictObject({ id: z.int(), enabled: z.boolean().optional(), title: z.string().optional(), prompt: z.string().optional(), schedule: schedule.optional(), threadMode, webSearch }),
  execute: async (args, ctx: AppToolCtx) => attempt(() => ctx.tasks.update(args.id, {
    enabled: args.enabled, title: args.title,
    prompt: args.prompt, schedule: args.schedule,
...
export const deleteTask = defineTool({
  ...
  execute: async (args, ctx: AppToolCtx) => attempt(() => { const id = args.id; if (!ctx.tasks.get(id)) throw new Error(`No task with id ${id}`); ctx.tasks.delete(id); return { deleted: id }; }),
});
