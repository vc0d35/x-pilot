...
export const scheduleTask: ToolModule<AppToolCtx> = {
  spec: {
    ...
    inputSchema: { type: 'object', properties: { title: { type: 'string' }, prompt: { type: 'string' }, schedule: scheduleSchema, threadMode: { type: 'string', enum: ['resume', 'new'] } }, required: ['title', 'prompt', 'schedule'], additionalProperties: false },
  },
  execute: async (args, ctx) => attempt(() => ctx.tasks.create({ title: String(args.title ?? ''), prompt: String(args.prompt ?? ''), schedule: (args.schedule as Record<string, unknown>) ?? {}, threadMode: args.threadMode === 'new' ? 'new' : 'resume' })),
};
...
export const updateTask: ToolModule<AppToolCtx> = {
  spec: {
    ...
    inputSchema: { type: 'object', properties: { id: { type: 'integer' }, enabled: { type: 'boolean' }, title: { type: 'string' }, prompt: { type: 'string' }, schedule: scheduleSchema, threadMode: { type: 'string', enum: ['resume', 'new'] } }, required: ['id'], additionalProperties: false },
  },
