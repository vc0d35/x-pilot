...
function view(rendered: string[], posts: Array<{ id: string }> = []) {
  ...
  return {
    ...
    callPreload: vi.fn(async (name: string, args: Record<string, unknown>) => {
      ...
      if (name === 'x_read_visible_posts') return ok(posts.slice(state.scrolls, state.scrolls + 2));
...
function ctx(likesMode: 'auto' | 'confirm', visibleIds: string[], bgIds: string[]) {
  const approvals = new ApprovalBroker(); const events: AgentEvent[] = []; approvals.onEvent((e) => events.push(e));
  const xview = view(visibleIds); const bg = view(bgIds, [{ id: 'a' }, { id: 'b' }, { id: 'b' }, { id: 'c' }]);
