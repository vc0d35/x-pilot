...
describe('ThreadState', () => {
  it('starts empty and round-trips through the file', () => {
    ...
    expect(s.get()).toEqual({ threadId: null, threadToolsHash: null });
    s.set({ threadId: 'T', threadToolsHash: 'h' });
    expect(JSON.parse(readFileSync(statePath(d), 'utf8'))).toEqual({ threadId: 'T', threadToolsHash: 'h' });
    expect(new ThreadState(statePath(d)).get()).toEqual({ threadId: 'T', threadToolsHash: 'h' });
  });
  ...
  it('patches one field at a time and can clear the thread', () => {
    ...
    expect(s.get()).toEqual({ threadId: 'T', threadToolsHash: 'h2' });
    s.set({ threadId: null, threadToolsHash: null });
    expect(s.get()).toEqual({ threadId: null, threadToolsHash: null });
  });
  ...
  it('migrates the thread out of an old settings.json on first run, once', () => {
    ...
    expect(ThreadState.beside(settingsPath).get()).toEqual({ threadId: 'old', threadToolsHash: 'old-hash' });
    expect(JSON.parse(readFileSync(statePath(d), 'utf8'))).toEqual({ threadId: 'old', threadToolsHash: 'old-hash' });
  ...
  it('writes nothing when the old settings file carried no thread', () => {
    ...
    expect(ThreadState.beside(settingsPath).get()).toEqual({ threadId: null, threadToolsHash: null });
  ...
  it('starts without a thread when either file is unreadable', () => {
    ...
    expect(ThreadState.beside(join(d, 'settings.json')).get()).toEqual({ threadId: null, threadToolsHash: null });
    const d2 = dir();
    writeFileSync(statePath(d2), '{ oops');
    expect(new ThreadState(statePath(d2)).get()).toEqual({ threadId: null, threadToolsHash: null });
  });

  it('ignores values of the wrong type', () => {
    const d = dir();
    writeFileSync(statePath(d), JSON.stringify({ threadId: 42, threadToolsHash: ['h'] }));
    expect(new ThreadState(statePath(d)).get()).toEqual({ threadId: null, threadToolsHash: null });
  });
});
