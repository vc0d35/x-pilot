...
export function handleNotification(method: string, params: unknown, phases: ItemPhases, h: NotificationHandlers): void {
  ...
  switch (method) {
    ...
    case 'item/agentMessage/delta':
      if (phases.commentary.has(p.itemId as string)) { h.emit({ type: 'thinking.delta', itemId: p.itemId as string, delta: p.delta as string }); return; }
      if (phases.writingItemId !== p.itemId) { phases.writingItemId = p.itemId as string; h.emit({ type: 'activity', activity: 'writing' }); }
...
function itemStarted(item: Record<string, unknown>, phases: ItemPhases, emit: (e: AgentEvent) => void): void {
  ...
  if (item.type === 'agentMessage' && item.phase === 'commentary') { phases.commentary.add(item.id as string); emit({ type: 'activity', activity: 'thinking' }); }
  ...
  if (item.type === 'mcpToolCall') emit({ type: 'activity', activity: 'tool', detail: `${item.server}/${item.tool}` });
  if (item.type === 'dynamicToolCall') emit({ type: 'tool.started', itemId: item.id as string, name: item.tool as string, args: item.arguments });
  if (item.type === 'commandExecution') emit({ type: 'tool.started', itemId: item.id as string, name: 'shell', args: item.command });
  if (item.type === 'mcpToolCall') emit({ type: 'tool.started', itemId: item.id as string, name: `${item.server}/${item.tool}`, args: item.arguments });
  if (item.type === 'webSearch') emit({ type: 'tool.started', itemId: item.id as string, name: 'web_search', args: { queries: webSearchQueries(item) } });
}
...
function itemCompleted(item: Record<string, unknown>, phases: ItemPhases, emit: (e: AgentEvent) => void): void {
  ...
  if (item.type === 'dynamicToolCall') {
    ...
    emit({ type: 'tool.completed', itemId: item.id as string, name: item.tool as string, success: item.success !== false, output: content.map((c) => c.text ?? '').join('\n') });
  }
  if (item.type === 'commandExecution') emit({ type: 'tool.completed', itemId: item.id as string, name: 'shell', success: item.exitCode === 0, output: (item.aggregatedOutput as string | null) ?? '' });
  if (item.type === 'mcpToolCall') emit({ type: 'tool.completed', itemId: item.id as string, name: `${item.server}/${item.tool}`, success: !item.error, output: JSON.stringify(item.result ?? item.error ?? null) });
  if (item.type === 'webSearch') emit({ type: 'tool.completed', itemId: item.id as string, name: 'web_search', success: true, output: (item.query as string | null) ?? webSearchQueries(item).join(' | ') });
}
