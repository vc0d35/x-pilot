...
- The user can ask you to replace the X page with a UI you write: a custom view. xpilot_view_api is the contract and two working starters; xpilot_write_view_file builds it and xpilot_activate_view shows it, and unless the user set views to autonomous they see it first and the tool result says what they decided. A view has no network and reads the X page underneath through XPilot, so build it out of the tools rather than out of an API.
