...
export class TasksStore {
  ...
  create(t: { title: string; prompt: string; schedule: TaskSchedule; threadMode: 'resume' | 'new'; webSearch?: boolean; nextRunAt: string | null }): ScheduledTask {
  ...
  update(id: number, patch: Partial<Pick<ScheduledTask, 'title' | 'prompt' | 'schedule' | 'threadMode' | 'threadId' | 'enabled' | 'webSearch' | 'lastRunAt' | 'lastStatus' | 'nextRunAt'>>): void {
  ...
  delete(id: number): void { this.db.prepare('DELETE FROM tasks WHERE id = ?').run(id); }
