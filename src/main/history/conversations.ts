...
export class ConversationsStore {
  ...
  appendEvent(threadId: string, event: AgentEvent): void {
    const now = stamp();
    const next = (
      this.db.prepare('SELECT COALESCE(MAX(seq), 0) + 1 AS n FROM conversation_events WHERE thread_id = ?').get(threadId) as { n: number }
    ).n;
    this.db
      .prepare('INSERT INTO conversation_events(thread_id, seq, event_json) VALUES (?, ?, ?)')
      .run(threadId, next, JSON.stringify(event));
    if (event.type === 'user.message')
      this.db
        .prepare("UPDATE conversations SET title = CASE WHEN title = '' THEN ? ELSE title END, updated_at = ? WHERE thread_id = ?")
        .run(titleFrom(event.text), now, threadId);
  ...
  listEvents(threadId: string): AgentEvent[] {
    return (
      this.db.prepare('SELECT event_json FROM conversation_events WHERE thread_id = ? ORDER BY seq').all(threadId) as Array<{
        event_json: string;
      }>
    ).map((r) => JSON.parse(r.event_json) as AgentEvent);
  }
  ...
  list(limit = 200): Conversation[] {
    return (
      this.db
        .prepare(
          `SELECT c.* FROM conversations c WHERE EXISTS (SELECT 1 FROM conversation_events e WHERE e.thread_id = c.thread_id)
  ...
  pruneEmpty(exceptThreadId: string | null): void {
    this.db
      .prepare(
        `DELETE FROM conversations WHERE (? IS NULL OR thread_id <> ?) AND NOT EXISTS (SELECT 1 FROM conversation_events e WHERE e.thread_id = conversations.thread_id)`,
      )
      .run(exceptThreadId, exceptThreadId);
  }
  ...
  applyRetention(policy: RetentionPolicy): RetentionResult {
    ...
    `,
        ...
        .all(policy.keepConversations, cutoff, recent, policy.keepThreadId ?? null, policy.keepThreadId ?? null) as Array<{
        thread_id: string;
      }>
    ).map((r) => r.thread_id);
    ...
    const events = (
      this.db.prepare(`SELECT COUNT(*) AS n FROM conversation_events WHERE thread_id IN (${holes})`).get(...victims) as { n: number }
    ).n;
    this.db.exec('BEGIN');
    try {
...
function rowToConversation(r: Record<string, unknown>): Conversation {
  return {
    ...
    toolsHash: (r.tools_hash as string | null) ?? null,
  };
}
