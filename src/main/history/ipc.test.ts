...
const post = (over: Record<string, unknown> = {}) => ({
  id: '1', url: 'https://x.com/a/status/1', authorHandle: 'a', authorName: 'A', text: 'hello', postedAt: null, kind: 'post', ...over,
});
...
function setup(opts: { now?: () => number } = {}) {
  ...
  const ipc = { on: (ch: string, l: (...a: never[]) => void) => { em.on(ch, l as (...a: unknown[]) => void); } };
...
describe('registerHistoryIpc', () => {
  ...
  it('drops payloads that exceed the caps or are not x.com status posts', () => {
    const { em, store } = setup();
    const send = (p: Record<string, unknown>) => em.emit(IPC.historyLiked, { sender: { id: 1 } }, { post: p, likedAt: '2026-09-01T00:00:00Z' });
  ...
  it('accepts twitter.com status urls', () => {
    const { em, store } = setup();
    em.emit(IPC.historyLiked, { sender: { id: 1 } }, { post: post({ url: 'https://twitter.com/a/status/1' }), likedAt: '2026-09-01T00:00:00Z' });
    expect(store.count()).toBe(1);
  });
  ...
  it('rate-limits a sender to 10 events per 10 s and warns on the rest', () => {
    ...
    for (let i = 0; i < 15; i++) em.emit(IPC.historyLiked, { sender: { id: 1 } }, { post: post({ id: String(100 + i), url: `https://x.com/a/status/${100 + i}` }), likedAt: '2026-09-01T00:00:00Z' });
    expect(store.count()).toBe(10);
    expect(warnings).toHaveLength(5);
    clock += 10_001;
    em.emit(IPC.historyLiked, { sender: { id: 1 } }, { post: post({ id: '200', url: 'https://x.com/a/status/200' }), likedAt: '2026-09-01T00:00:00Z' });
  ...
  it('rate-limits per sender, not globally', () => {
    ...
    for (let i = 0; i < 12; i++) em.emit(IPC.historyLiked, { sender: { id: 1 } }, { post: post({ id: String(100 + i), url: `https://x.com/a/status/${100 + i}` }), likedAt: '2026-09-01T00:00:00Z' });
    for (let i = 0; i < 3; i++) em.emit(IPC.historyLiked, { sender: { id: 2 } }, { post: post({ id: String(300 + i), url: `https://x.com/a/status/${300 + i}` }), likedAt: '2026-09-01T00:00:00Z' });
  ...
  it('adds later X windows to one set of listeners instead of stacking new ones', () => {
    ...
    expect(store.count()).toBe(1);
    em.emit(IPC.historyLiked, { sender: { id: 7 } }, { post: post({ id: '2', url: 'https://x.com/a/status/2' }), likedAt: '2026-09-01T00:00:00Z' });
    expect(store.count()).toBe(1);
  });
});
