...
describe('pdfFileName', () => {
  it('builds a safe, dated file name', () => {
    ...
    expect(
      pdfFileName({
        ...
        author: 'alice',
        title: 'On Compilers: a long essay about many things that goes on',
        id: '111',
      }),
  ...
  it('cannot be talked into a traversing file name', () => {
    expect(pdfFileName({ date: new Date('2026-09-07T10:00:00Z'), author: 'alice', title: 'hi', id: '../../evil' })).toBe(
      '2026-09-07-alice-hi-evil.pdf',
    );
    expect(pdfFileName({ date: new Date('2026-09-07T10:00:00Z'), author: 'alice', title: 'hi', id: '111/../../x' })).not.toContain('/');
  });
});
