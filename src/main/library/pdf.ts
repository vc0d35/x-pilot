...
export async function exportPdf(opts: { url: string; outDir: string; timeoutMs?: number }, deps: PdfDeps): Promise<{ path: string; title: string }> {
  ...
  try {
    ...
    if (landed !== requested) throw new Error(`The page moved during the export: expected ${requested}, got ${win.getURL?.() ?? 'an unknown URL'}`);
    const requestedId = /\/(\d{1,25})$/.exec(requested)?.[1] ?? '';
    if (String(meta.id ?? '') !== requestedId) throw new Error(`The page reported a different post (${meta.id}) than the one exported (${requestedId})`);
    const urlAuthor = /^https:\/\/x\.com\/([A-Za-z0-9_]{1,15})\//.exec(requested)?.[1] ?? '';
    const author = urlAuthor === 'i' ? String(meta.author ?? '') : urlAuthor;
    const title = String(meta.title ?? '').replace(/\s+/g, ' ').trim().slice(0, 200);
