...
describe('textLines', () => {
  it('splits at block boundaries, collapses whitespace, skips buttons and repeated lines', () => {
    const e = el('<div><div><span>Trending in </span><span>Netherlands</span></div><div><span>#tag</span><span>#tag</span></div><button><svg></svg>x</button><div>  </div></div>');
...
describe('widgetItem', () => {
  it('names a trend by the line that is neither context nor count', () => {
    expect(widgetItem(el('<div data-testid="trend"><div>Politics · Trending</div><div>Rusland</div><div>12.3K posts</div></div>'))).toEqual({ title: 'Rusland', detail: 'Politics · Trending · 12.3K posts' });
    expect(widgetItem(el('<div data-testid="trend"><div>Trending in Netherlands</div><div>Nijkerk</div></div>'))).toEqual({ title: 'Nijkerk', detail: 'Trending in Netherlands' });
  });
  it('uses the first line for a news headline even when it mentions trending', () => {
    expect(widgetItem(el('<div data-testid="news_sidebar_article_x"><div>Why Trending Topics Fade</div><div>1 hour ago · Other · 9 posts</div></div>'))).toEqual({ title: 'Why Trending Topics Fade', detail: '1 hour ago · Other · 9 posts' });
  });
...
describe('extractWidgets', () => {
  it('groups Explore and sidebar items under their headings, resetting at empty cells, and dedupes titles', () => {
    ...
    expect(sections.map((s) => [s.heading, s.items.map((i) => i.title)])).toEqual([
      ["Today's News", ['Meme Contrasts Vibe Coding Chaos with Neat Engineering', "Blockbuster's 2001 Shift from VHS to DVDs Marks 25 Years"]],
...
describe('findNewPostsButton', () => {
  it('finds the pill by its label and parses the count', () => {
    document.body.innerHTML = '<div data-testid="primaryColumn"><div data-testid="cellInnerDiv"><button aria-label="Retry">Retry</button></div><div data-testid="cellInnerDiv"><button><div><span>Show 1,204 posts</span></div></button></div></div>';
