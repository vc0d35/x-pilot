...
export const readComposer: ToolModule<PreloadCtx> = {
  spec: { name: 'x_read_composer', description: 'Reads the open post composer: whether it is open, its current text, and whether Post is enabled.', inputSchema: { type: 'object', properties: { timeoutMs: { type: 'integer', default: 5000 } }, additionalProperties: false }, annotations: { readOnlyHint: true, internal: true } },
  execute: async (args) => {
...
export const typeInComposer: ToolModule<PreloadCtx> = {
  spec: { name: 'x_type_in_composer', description: 'Types text into the open composer (used when the compose URL did not prefill it).', inputSchema: { type: 'object', properties: { text: { type: 'string' } }, required: ['text'], additionalProperties: false }, annotations: { internal: true } },
  execute: async (args) => {
...
export const clickPostButton: ToolModule<PreloadCtx> = {
  spec: { name: 'x_click_post_button', description: 'Clicks the Post button of the open composer and reports the confirmation toast and new post URL if shown. Internal: main calls this after approval.', inputSchema: { type: 'object', properties: { timeoutMs: { type: 'integer', default: 8000 } }, additionalProperties: false }, annotations: { destructiveHint: true, internal: true } },
  execute: async (args) => {
