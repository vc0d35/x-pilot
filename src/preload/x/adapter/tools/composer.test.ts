...
describe('composer tools', () => {
  ...
  it('x_read_composer fails fast when no composer is open', async () => {
    expect(await readComposer.execute({ timeoutMs: 30 }, ctx)).toEqual({ success: false, error: 'No composer is open' });
  });
  ...
  it('x_type_in_composer inserts text through execCommand', async () => {
    ...
    const r = await typeInComposer.execute({ text: 'hello there' }, ctx);
  ...
  it('x_click_post_button clicks and reads the toast link', async () => {
    ...
    const r = await clickPostButton.execute({ timeoutMs: 500 }, ctx);
  ...
  it('x_click_post_button refuses when the button is disabled', async () => {
    ...
    expect(await clickPostButton.execute({}, ctx)).toEqual({ success: false, error: 'Post button is disabled (empty draft or over the length limit)' });
  });
});
