...
- Custom views security review fixes: WebRTC removed from a view's world before any of its code runs (no content policy governs it, and no Chromium switch takes it away), so a view really has no network; every tool call a view makes is a `view:<tool>` row on the agent's stream and in the conversation; a card a view raised says which view asked and is drawn as its own; the four account writes always confirm from a view, whatever the autonomous settings say, and `x_compose_post` asks before it opens a composer; a view that has not been kept yet may only read and draw; any approval card expands a collapsed sidebar; a view's `x_navigate` is limited to https on x.com and twitter.com, never a short link; taking a view off screen leaves the X page on `/home` rather than on a composer it filled; downloads from a view are refused; `xpilot_view_inspect` is bounded whatever the view's markup does; and on the `xpilot://` scheme, views are served only on the views session, symlinks are never served, every response carries the view CSP, and `.md` is served as markdown.
